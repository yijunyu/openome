...
/**
 * The panel consists of a horizontal scroll bar, a vertical scrool car and a * <code> GraphicViewCanvas </code>. There is a class * <code> ScrollingPane </code> from JDK with similiar functionalities. But this * class (<code> ScrollingPanel </code>) optimizes screen redrawing by only * drawing the visible elements and links. The drawing optimization is * encapsulated in the <code> GraphicViewCanvas </code> class. *  * @author Ying Shi * @version 28 Aug 1998 *  * @see OME * @see GraphicViewFrame * @see GraphicViewCanvas *  * NOTE: First OME developer to successfully remove ScrollingPanel from the OME * source (to be replaced with a lightweight Swing component) wins a large * cookie. We believe that the scroll bars in GVC are what cause the "hidden * menu" bug.
 */
...
class ScrollingPanel extends JPanel {
	 ...
	 *            the graph view data.
	 * @param gframe
	 *            the graph view frame window.
	 * @param gvc
	 *            the canvas we hook with.
	 */
	...
	class ScrollingPaneAdjustmentAdapter implements AdjustmentListener {
		public void adjustmentValueChanged(AdjustmentEvent e) {
			...
			if (e.getAdjustable() == sbv)
				mc.setSbvValue(value);
			else
				mc.setSbhValue(value);
			mc.repaint();
		}
	}
