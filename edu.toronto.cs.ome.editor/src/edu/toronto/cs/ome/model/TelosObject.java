...
public abstract class TelosObject implements ModelObject, TelosFunctionality 
{

	/**
	 * 
	 * @uml.property name="modelattributes"
	...
	/**
	 * 
	 * @uml.property name="modelvalueattributes"
	 * @uml.associationEnd multiplicity="(0 -1)"
	 */
    ...
    public class TelosValueAttribute implements ModelValueAttribute {
     ...
     * attributes, however. */
    public class TelosAttribute implements ModelAttribute {
	private Attribute attribute;
	String name;
	String [] cats;
	...
	public Iterator getPossibleTargets() {
	    ...
	    PropositionOrPrimitive to=parentatt[0].to();
	    if (to instanceof TelosParserIndividual) {
			Proposition targets[] = //ind.allInstances();
			    ((TelosParserIndividual)to).allInstances();//Updated by Xiao X. Deng Dec 28., 2004
			return Arrays.asList(targets).iterator();
	    }
	    }		
	    //List list = new LinkedList();
	    List list=new Vector(10,10);
...
//			    new TelosValueAttribute(parentatt[i].label());
    ...
    public String getName() {
	   try {
		TelosString name =
			(TelosString) individual.directAttributes(NAMECAT,NOLABEL)[FIRST].to();
