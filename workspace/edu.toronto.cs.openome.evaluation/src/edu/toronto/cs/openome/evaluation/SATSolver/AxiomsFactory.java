...
public class AxiomsFactory {
	...
	public LinkAxioms createLinkAxiom(Vector<Intention> sources, Intention targ, Vector<Link> l, String type, DualHashMap<Integer, Intention> dhm) {
		...
		if (type.equals("Make")) {
			MakeContributionLinkAxioms dla = new MakeContributionLinkAxioms(sources, targ, l, dhm);
			return dla;
		}
		if (type.equals("Help")) {
			HelpContributionLinkAxioms dla = new HelpContributionLinkAxioms(sources, targ, l, dhm);
			return dla;
		}
		if (type.equals("Unknown")) {
			UnknownContributionLinkAxioms dla = new UnknownContributionLinkAxioms(sources, targ, l, dhm);
			return dla;
		}
		if (type.equals("Hurt")) {
			HurtContributionLinkAxioms dla = new HurtContributionLinkAxioms(sources, targ, l, dhm);
			return dla;
		}
	...
	public IntentionAxioms createIntentionAxiom(Intention targ, String type, DualHashMap<Integer, Intention> dhm) {
		if (type.equals("Invariant")) {
			InvariantAxioms ax = new InvariantAxioms(targ,dhm);
			return ax;
		}
		if (type.equals("Constraint")) {
			ConstraintAxioms ax = new ConstraintAxioms(targ,dhm);
			return ax;
		}
		if (type.equals("Target")) {
			TargetAxioms ax = new TargetAxioms(targ,dhm);
			return ax;
		}
