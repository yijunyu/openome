...
/**
 * TelosLink is a Telos implementation of OMELink. It is an interface to links
 * within the Telos KB. Synchronization to the KB will ultimately be handled
...
public class TelosLink extends TelosObject implements ModelLink {
	...
	/**
	 * Returns this Telos link's type. We are currently forcing that our links
	 * and elements be simpleclasses, and that they be singly instantiated.
	 */
	 ...
	 * 
	 * @param newtype
	 *            the new type that should be assigned to this Telos link
	 */
	public void setType(Object newtype) throws Exception {
		Proposition newtypes[] = { toProposition(newtype)};
		...
		if (newtype != type) {
			...
			this.type = toProposition(newtype);
		}
	}
	...
	/**
	 * Returns the unique (within its view) integer ID of this Telos link. The
	 ...
	 * 
	 * @param id
	 *            the integer id to be assigned to this Telos link
	 */
	 ...
	 * 
	 * @param e
	 *            the destination object to be assigned to this Telos link
	 */
	public void setTo(OMEObject e) throws Exception {
		...
		try {
			Proposition p = kb.newAttribute(individual, TOCAT, TO,
					((TelosObject) e).getIndividual());
			//e.getLinks().add(this);
			D.a(getIndividual());
			e.addLink(this);
	 ...
	 * 
	 * @param o
	 *            the source to be assigned to this Telos element
	 */
	public void setFrom(OMEObject o) throws Exception {
		...
		try {
			Proposition p = kb.newAttribute(individual, FROMCAT, FROM,
					((TelosObject) o).getIndividual());
			//o.getLinks().add(this);
			o.addLink(this);
	 ...
	 * 
	 * @param kb
	 *            the knowledge base where this link would be stored
	 * @param framework
	 *            the Telos framework on which this model is based on
	 * @param model
	 *            the Telos model in which this Telos link is in.
	 * @param id
	 *            a unique ID (within a view) for this Telos link
	 */
	public TelosLink(KB kb, TelosFramework framework, TelosModel model, int id,
			Object type) throws Exception {
		super();
	 ...
	 *            a unique ID (within a view) for this Telos link
	 * @param proposition
	 *            the proposition which represents this link
	 */
	public TelosLink(KB kb, TelosFramework framework, TelosModel model, int id,
			Proposition p) throws Exception {
		super((Individual) p);
