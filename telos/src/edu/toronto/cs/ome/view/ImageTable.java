...
public class ImageTable extends JComponent {
	...
	/**
	 * Return the specified image and size (loading it from the scaleTable, if
	 * it exists in scaleTable, otherwise creating it and adding it to the
	 * scaleTable).
	 * 
	 * @param image
	 *            the image required
	 * @param scale
	 *            the scale factor needed for the image
	 */
	 ...
	 *            the image required
	 * @param widthfactor
	 *            width multiple desired
	 * @param heightfactor
	 *            height multiple desired
	 */
	public Image getScaling(Image image, double widthfactor, double heightfactor) {
		try {
		Image result = (Image) scaletable.get(new Pair(image, new Pair(
				new Double(widthfactor), new Double(heightfactor))));
		if (result == null) {
			result = new BufferedImage((int) Math.rint(image.getWidth(this)
					* (widthfactor)), (int) Math.rint(image.getHeight(this)
					* (heightfactor)), BufferedImage.TYPE_INT_ARGB);
	 ...
	 * or width. If the provided height is 0, the image will be sized to the
	 * specified width, and the image will retain its original height/width
	 * ratio. Providing a width of 0 work similarly. If both width and height
	 * are 0, the original image is returned.
	 * 
	 * @param im
	 *            the image which should be rescaled
	 * @param width
	...
	/**
	 * Return the image with specified image and rotation (loading it from the
	 * scaleTable, if it exists in scaleTable, otherwise creating it and adding
	 * it to the scaleTable).
	 * 
	 * @param image
	 *            the image to be scaled
	 * @param rotation
	 *            the rotation needed for the image
	 */
	public Image getRotation(Image image, float rotation) {

		Image result = (Image) rotationtable.get(new Pair(image, new Float(
				rotation)));

		if (result == null) { // we must fill in that entry
			result = new BufferedImage(image.getWidth(this), image
					.getHeight(this), BufferedImage.TYPE_INT_ARGB);
			Graphics2D g = ((BufferedImage) result).createGraphics();

			g.rotate((double) rotation, (double) image.getWidth(this) / 2.0,
			...
			//	    D.o("created rotation: "+rotation*360.0/(2.0*Math.PI) + "
			// degrees");
		}
		return result;
	}
	 ...
	 * filename, *can* safely be used for this, but you will take a performance
	 * hit, so if you have some other handle lying around, then use it instead.
	 * This method will *immediately* load the image, ie: wait to be done before
	 * returning.
	 * 
	 * @param filename
	 *            a possible handle for the image
	 * @param handle
	 *            an alternative handle to be used, if one chooses not to use
	 *            the filanem as the handle.
	 */
	public Image putImage(String filename, Object handle) {
		//D.o("filename="+filename);
		try {
			//D.o(""+this.getClass().getResource("/resources/"+filename));
			Image image = Toolkit.getDefaultToolkit().getImage(
					this.getClass().getResource("/resources/" + filename));
	 ...
	 * 
	 * @param image
	 *            the image whose height we want
	 */
	 ...
	 * Useful for images that represent links.
	 * 
	 * @param im
	 *            the image which the horizontal line should be drawn through
	 */
	 ...
	 * 
	 * @param im
	 *            the image which the dashed horizontal line should be drawn
	 *            through.
	 */
	public Image drawDashedHLine(Image im) {
		BufferedImage bi = null;
		if (getWidth(im) == 0 || getHeight(im) == 0)
			return new BufferedImage(20, 20, BufferedImage.TYPE_INT_ARGB);
		bi = new BufferedImage(getWidth(im), getHeight(im),
				BufferedImage.TYPE_INT_ARGB);
		Graphics2D g = (Graphics2D) bi.getGraphics();
		g.drawImage(im, 0, 0, this);
		g.setColor(Color.black);
