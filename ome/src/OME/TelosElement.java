 ...
 */
class TelosElement extends TelosObject implements ModelElement {
	...
	public Iterator iterator() {
		...
		for (int c=0; c<children.length; c++) {
		    ll.add(model.getElement((Proposition)children[c].to()));
		}
		return ll.iterator();
...
//	    kb.newAttribute(individual, PARENTCAT, NOLABEL, target);
		...
		D.o("Setting the parent of this element: " + id + " " +
			oe.getID());
		Proposition p = kb.newAttribute(individual, PARENTCAT, NOLABEL,
			((TelosElement)oe).getIndividual());
		D.o("Set the parent of this element");
	    } catch (Exception e) {
