...
/** Contains all the information needed to maintain a Graphic View Link.
 *  A GraphicViewLink is a link between two nodes in a GraphView. Links have an
 *  annotation, a source, and destination, and a type which is used to find the
 *  image out of the imagetable. 
 */
class GraphicViewLink extends GraphicViewObject implements ViewLink {

    private String name=null;		    // text annotation.
    private int id=0;			    // should this be == to OMELink's?
    ...
    private QuadCurve2D.Double Quad1, Quad2;
    Point2D.Double start1, end1, control1;
    Point2D.Double start2, end2, control2;
    Rectangle ctrl1, ctrl2;
    private int init=0;
    private Color color;
     ...
     *  @param link 	the ModelLink which is been represented by this
     *			graphic view link
	...
	nameattribute = new GraphicViewTextAttribute(50,
		GraphicViewTextAttribute.LEFT_JUSTIFIED, 25, -10);
...
/*    protected int maxPrefTextWidth() {
  ...
  */  
     ...
     *  position relative to our image bounds.  Our text is just some
     *  constants away from our image. */
/*    protected void setTextBounds(int width, int height) {
	final int rightoffset = 10;
	final int topoffset = -15;
	textbounds = new Rectangle(getImageBounds().width
		+ rightoffset, topoffset, width, height);
    }
*/
...
//	return id;
	...
	o = view.getViewObject((ModelObject)getModelLink().getTo());
		
	//D.o("GraphicViewLink.getTo returning null? "+(e == null));
	return o;
     ...
     *  @param vo	the destination to be be assigned
     */
	...
	//D.o("GraphicViewLink.getFrom returning null? "+(e == null));
	...
	getstart1().setLocation(from.getImageBounds().x+from.getImageBounds().getWidth()/2,
	...
	Point2D.Double src = new Point2D.Double(from.getImageBounds().x+from.getImageBounds().getWidth()/2,

		from.getImageBounds().y+from.getImageBounds().getHeight()/2);

	    Point2D.Double dst = new Point2D.Double(to.getImageBounds().x+to.getImageBounds().getWidth()/2,
	    ...
	    getctrl2().setLocation((int)((getcontrol2().x)-4), 
	...
	OMEModel model = link.getModel();
	ModelLink l = model.createLink();
	GraphicViewLink gvl = new GraphicViewLink(l,view);
	
	// start with a bitwise clone then correct it.
	//gvl.setName(name);
	gvl.setType(getType());	
    ...
    /** Returns whether this graphic view link is hidden. For now we are 
     *  hidden if either end is hidden, or if hidden is set. 
     */
    ...
    /** Sets the icon for the object.  Passing <code>null</code> for
     * icon de-iconifies this object.  We had better get a GraphicViewObject
     * that is in our view.
     */
    ...
    /** Returns the bounding box for this link. This is just for the image in
     * the centre of the link.  Let's try moving to 2/3 of the way up the
     * link.*/
	...
	Rectangle bounds = new Rectangle(
		c.x-(int)(getScale()*it.getWidth(image)/2),
		c.y-(int)(getScale()*it.getHeight(image)/2),
		(int)(getScale()*it.getWidth(image)),
		(int)(getScale()*it.getHeight(image)));
	return bounds;
    }
    ...
    /** Returns the least upper rectangular box encompassing the area dirtied
     *  by moving this link. */
    ...
    /** Returns the angle between the ends of this link. The result is
     *  quantized onto ROTATIONS increments to allow caching of the result. 
     */
    public float getRotation() {
	Point a = new Point((int)((GraphicViewObject)getFrom()).getImageBounds().x
		+ (int)((GraphicViewObject)getFrom()).getImageBounds().getWidth()/2,
		(int)((GraphicViewObject)getFrom()).getImageBounds().y
		+(int)((GraphicViewObject)getFrom()).getImageBounds().getHeight()/2);
	Point b = new Point((int)((GraphicViewObject)getTo()).getImageBounds().x
		+(int)((GraphicViewObject)getTo()).getImageBounds().getWidth()/2,
		(int)((GraphicViewObject)getTo()).getImageBounds().y 
		+ (int)((GraphicViewObject)getTo()).getImageBounds().getHeight()/2);
